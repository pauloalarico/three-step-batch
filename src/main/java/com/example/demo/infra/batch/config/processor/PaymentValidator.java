...
@Component
public class PaymentValidator implements ItemProcessor<Payment, ProcessedPayment> {


    @Override
    public @Nullable ProcessedPayment process(Payment item) throws Exception {
        if(item.getStatus().equals(PaymentStatus.VENCIDA)) {
            return new ProcessedPayment(item, com.example.demo.domain.enums.PaymentValidator.INVALID);
        }

        return new ProcessedPayment(item, com.example.demo.domain.enums.PaymentValidator.VALID);
    }
}
